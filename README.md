...
## Upgrade from 2.x
With Drupal 10, domain_site_settings and domain_theme_switch are deprecated and
are replaced by domain_config. This causes breaking changes.
- Upgrade to 2.5.y and run update hooks.
This will convert favicons and robotstxt settings to use domain_config.
- Recreate theme selection per domain at /admin/appearance
if iq_multidomain_domain_theme_switch was used before.
...
#### Notes
- The sub module prefixes pattern ids from selected themes with the theme name
(except for the global default theme).
...
> It is also advised to enable the www-redirect option and
to not set the `www` or non-`www` domain in the ingress respectively.
